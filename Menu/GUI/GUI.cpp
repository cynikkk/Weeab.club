...
#include <algorithm>
#include "../../Main/SDK/SDK/tinyxml2.h"
#include <sstream>
...
#define UI_CURSOROUTLINE    Color(27,27,27,255)
...
#define UI_COL_MAIN			Color(123, 101, 179, 255)
#define UI_COL_SHADOW		Color(123, 101, 179, 255)
#define COL_WHITE           Color(123, 101, 179, 255)
#define UI_COL_MAINE        Color(123, 101, 179, 255)
#define UI_COL_MAINDARK     Color(113, 236, 159, 255)
#define UI_COURSOUR			Color(255, 255, 255, 255)
#define UI_COL_FADEMAIN     Color(123, 101, 179, 255)
#define UI_COL_SHADOW		Color(123, 101, 179, 255)
#define UI_COL_CLIENTBACK   Color(123, 101, 179, 255)
#define UI_COL_TABSEPERATOR Color(123, 101, 179, 255)
#define UI_COL_TABTEXT      Color(123, 101, 179, 255)
...
void CGUI::Update()
{
	...
	for (auto window : Windows)
	{
		if (window->m_bIsOpen)
		{
			...
			Render::Text((centerW * 2) - 95, 25, Color(123, 101, 179, (MenuAlpha - 1)), Render::Fonts::ESP, "weeab.club");
		...
		{
		//Render::Text((centerW)-10, (centerh * 2) - 50, Color(0, 250, 150, (MenuAlpha - 1)), Render::Fonts::ESP, "*09/09 Beta*");
		}
	}
}
...
bool CGUI::DrawWindow(CWindow* window)
{
	...
	if (TabCount) // If there are some tabs
	{
		...
		for (int i = 0; i < TabCount; i++)
		{
			...
			Color txtColor = Color(123, 101, 179, (MenuAlpha / 1.1));
			if (window->SelectedTab == tab)
			{
				RECT TextSize = Render::GetTextSize(Render::Fonts::Tabs, tab->Title.c_str());
				Render::gradient_verticle(TabArea.left, TabArea.top, TabArea.right, TabArea.bottom + TextSize.bottom - 10, Color(110, 101, 150, MenuAlpha), Color(7, 7, 7, MenuAlpha));
				txtColor = Color(cr, cg, cb, (MenuAlpha / 1.1));
			}
			else if (IsMouseInRegion(TabArea))
			{
				txtColor = Color(230, 230, 230, (MenuAlpha / 1.1));
			}
			RECT TextSize = Render::GetTextSize(Render::Fonts::Tabs, tab->Title.c_str());
	...
	Render::Outline(window->m_x - 2, window->m_y - 2, window->m_iWidth + 4, window->m_iHeight + 4, Color(123, 101, 179, MenuAlpha));
	Render::Outline(window->m_x - 3, window->m_y - 3, window->m_iWidth + 6, window->m_iHeight + 6, Color(80, 80, 80, MenuAlpha)); // test
	Render::Outline(window->m_x - 4, window->m_y - 4, window->m_iWidth + 8, window->m_iHeight + 8, Color(110, 101, 150, MenuAlpha));
	Render::Outline(window->m_x - 5, window->m_y - 5, window->m_iWidth + 10, window->m_iHeight + 10, Color(30, 30, 30, MenuAlpha));
	Render::Outline(window->m_x - 6, window->m_y - 6, window->m_iWidth + 12, window->m_iHeight + 12, Color(123, 101, 179, MenuAlpha));
	Render::Outline(window->m_x - 7, window->m_y - 7, window->m_iWidth + 14, window->m_iHeight + 14, Color(110, 101, 120, MenuAlpha));
...
void CGUI::SaveWindowState(CWindow* window, std::string Filename)
{
	...
	tinyxml2::XMLElement *Root = Doc.NewElement("weeab.club");
	...
	{
		MessageBox(NULL, "Unable to save config file. Please try again", "weeab.club", MB_OK);
	}
}
