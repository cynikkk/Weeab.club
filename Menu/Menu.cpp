...
void save_callback()
{
	...
	std::string config_directory = "weeabcfg\\cfg\\";
	...
	Interfaces::CVar->ConsoleColorPrintf(Color(30, 255, 30, 255), "weeab.club ");
...
void list_configs() {
	...
	file_in.open("weeabcfg\\cfg\\weeabcfg_configs.txt");
	if (file_in.fail()) {
...
void add_config() {
	std::fstream file;
	file.open("weeabcfg\\cfg\\weeabcfg_configs.txt", std::fstream::app);
	if (file.fail()) {
		std::fstream("weeabcfg\\cfg\\weeabcfg_configs.txt");
		file.open("weeabcfg\\cfg\\weeabcfg_configs.txt", std::fstream::app);
	}
...
void remove_config() {
	...
	std::ofstream ofs("weeabcfg\\cfg\\weeabcfg_configs.txt", std::ios::out | std::ios::trunc);
	ofs.close();
	std::fstream file;
	file.open("weeabcfg\\cfg\\weeabcfg_configs.txt", std::fstream::app);
	if (file.fail()) {
