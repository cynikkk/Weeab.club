...
#include "../../../Hacks/Features/AutoWall/Autowall.h"
...
Vector GetHitboxPosition(IClientEntity* pEntity, int Hitbox)
{
	...
	if (pEntity->SetupBones(matrix, 128, 0x100, 0.f))
	{
		...
		mstudiobbox_t* hitbox_head = set->GetHitbox(1);
			...
			if (Options::Menu.RageBotTab.Multienable.GetState())
			{
				...
				for (int i = 0; i <= iCount; i++)
				{
					...
					default:
						vTargetPos = vCenter; break;
					case 1:
						vTargetPos = (Point[7] + Point[1]) * Options::Menu.RageBotTab.Multival2.GetValue() / 100; break;
					case 2:
						vTargetPos = (Point[3] + Point[4]) * Options::Menu.RageBotTab.Multival2.GetValue() / 100; break;
					case 3:
						vTargetPos = (Point[4] + Point[0]) * Options::Menu.RageBotTab.Multival2.GetValue() / 100; break;
					case 4:
						vTargetPos = (Point[2] + Point[7]) * Options::Menu.RageBotTab.Multival2.GetValue() / 100; break;
					case 5:
						vTargetPos = (Point[6] + Point[7]) * Options::Menu.RageBotTab.Multival2.GetValue() / 100; break;
					case 6:
						vTargetPos = (Point[5] + Point[3]) * Options::Menu.RageBotTab.Multival2.GetValue() / 100; break;
					case 7:
						vTargetPos = (Point[1] + Point[2]) * Options::Menu.RageBotTab.Multival2.GetValue() / 100; break;
					}
					return vTargetPos;
				}
			}
