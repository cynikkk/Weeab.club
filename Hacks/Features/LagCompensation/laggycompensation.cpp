...
void createmove_backtrack::tickoverride(IClientEntity *player, QAngle angles, float simtime)
{
	int idx = player->GetIndex();
	LagRecord overwrite_record;
	auto& m_LagRecords = this->m_PlayerTrack[player->GetIndex()];

	overwrite_record.SaveRecord(player);
	overwrite_record.m_flSimulationTime = simtime;
	m_LagRecords.emplace_back(overwrite_record);
}
void createmove_backtrack::SaveRecord(IClientEntity* player)
{
	m_nFlags = player->getFlags();
	m_flSimulationTime = player->GetSimulationTime();
}
